...
export const useSEO = (customConfig?: Partial<SEOConfig>) => {
  ...
  useEffect(() => {
    ...
    updateMetaTag('og:url', finalConfig.canonical || `${window.location.origin}${location.pathname}`, 'property');
    ...
    canonicalLink.setAttribute('href', finalConfig.canonical || `${window.location.origin}${location.pathname}`);
  ...
  }, [location.pathname, currentLang, finalConfig]);
